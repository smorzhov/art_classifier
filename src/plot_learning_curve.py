...
def make_learning_curves(learning_curve_path, train_log, test_log):
    ...
    train_loss, = ax1.plot(
        train_log['NumIters'], train_log['loss/loss'], color='red', alpha=.5)
    test_loss, = ax1.plot(
        test_log['NumIters'], test_log['loss/loss'], linewidth=2, color='green')
    ...
    test_accuracy, = ax2.plot(
        test_log['NumIters'], test_log['accuracy@1'], linewidth=2, color='blue')
